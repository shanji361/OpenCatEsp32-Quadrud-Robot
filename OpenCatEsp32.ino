...
void loop()
{
  ...
  readEnvironment(); // update the gyro data, updates yaw ypr[0] (the z axis in turn)
  
  //  //— special behaviors based on sensor events
  print6Axis(); //print to Bluetooth or USB serial 
...
//
//  // Begin turning
//  tQueue->addTask(T_SKILL, "balance"); // Disable gait stabilization temporarily if needed
//  delay(200);
//
//  while (abs(angleDiff(currentYaw, targetYaw)) > 5.0) {
//    tQueue->addTask(T_SKILL, "vtR"); // vtR = turn right in place
